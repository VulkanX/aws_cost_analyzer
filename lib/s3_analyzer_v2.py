...
class S3_analyzer:
        ...
        for region in region_list:
            ...
            for bucket in s3_buckets:
                ...
                if 'Datapoints' in bucket_size_data and len(bucket_size_data['Datapoints']) > 0:
                    ...
                    enc_res = s3_client.get_bucket_encryption(Bucket=bucket['Name'])["ServerSideEncryptionConfiguration"]
                    bucket_encryption = str(enc_res['Rules'][0]['ApplyServerSideEncryptionByDefault']['SSEAlgorithm']) + "|" + str(enc_res['Rules'][0]['BucketKeyEnabled'])

                    # Get bucket versioning
                    version_res = s3_client.get_bucket_versioning(Bucket=bucket['Name'])
                    # print(version_res)
                    ...
                    # Get Lifecycle Policies
                    try:
                        lifecycle_res = s3_client.get_bucket_lifecycle_configuration(Bucket=bucket['Name'])
                    ...
                    try:
                        replication_res = s3_client.get_bucket_replication(Bucket=bucket['Name'])
                    ...
                    try:
                        pab_res = s3_client.get_public_access_block(Bucket=bucket['Name'])
                        if pab_res['PublicAccessBlockConfiguration']['BlockPublicAcls'] == True and pab_res['PublicAccessBlockConfiguration']['BlockPublicPolicy'] == True:
                            bucket_public = 'Private'
                    except botocore.exceptions.ClientError as e:
                        # Check Policy
                        try:
                            policy_res = s3_client.get_bucket_policy_status(Bucket=bucket['Name'])
                            if policy_res['PolicyStatus']['IsPublic'] == False:
                                bucket_public = 'Private'
                        except botocore.exceptions.ClientError as e:
                            bucket_public = 'Public'
    ...
    def csv(self):
        ...
        with open('s3.csv', 'w', newline='') as csvfile:
            fieldnames = ['Region', 'BucketName', 'BucketSize', 'BucketItems', 'BucketCost', 'BucketGlacier', 'BucketEncryption', 'BucketVersioning', 'BucketLifecycle', 'BucketReplication', 'BucketPublic']
